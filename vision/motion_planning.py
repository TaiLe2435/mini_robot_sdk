...
if __name__ == "__main__":
    ...
    while inputVideo.isOpened():
        ...
        if ids is not None and len(ids) > 0:
            ...
            if traj == "a":
                kP = 0.65
                line_error = (straight_left[1] - y_center)
                pos_error = (straight_right[0] - x_center)
                
                velocity_left = pos_error * kP + kP * line_error;
                if velocity_left > 400:
                    velocity_left = 400
                elif velocity_left < -400:
                    velocity_left = -400
                    
                velocity_right = pos_error * kP - kP * line_error;
            ...
            # # for straight line
            # if ctr < 100: # should change to stop when it reaches point rather than ctr
            #     try:
            #         ctr += 1
